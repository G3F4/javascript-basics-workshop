...
Assign a unique identifier to `div` element, find it, and change it text content to empty string.
Then create the new element and append it to DOM.
...
5. Check browser, test if clicking in the button with text `Play game!` changes view
...
1. Game timer. Add a timer which will count down from fixed value
   1a. Add way to setup time on first view
